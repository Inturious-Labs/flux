...
cleanup_on_exit() {
    # Only restore if we're not in a normal exit path
    if [ -f "$HOME/Library/Application Support/Cursor/User/settings.json.backup" ]; then
        source "$SCRIPT_DIR/lib/session.sh"
        end_writing_session 2>/dev/null || true
    fi
}
...
case "${1:-}" in
    ...
    "git")
        if [ -n "$2" ]; then
            ...
            echo -e "${RED}Usage: $0 git [dsc|sb|hy]${NC}"
        fi
        exit 0
        ;;
    "help"|"--help"|"-h")
        ...
        echo -e "  ${GREEN}$0 git [site]${NC}     - Check git status for site (dsc|sb|hy)"
        echo ""
        ...
        if [ -z "$current_site" ]; then
            ...
            case "$choice" in
                1)
                    ...
                    git_aware_site_selection "$current_site"
                    ;;
                2)
                    current_site="sb"
            ...
            case "$action" in
                n|new)
                    ...
                    if [ $? -eq 0 ]; then
                        ...
                        if [[ ! "$open_choice" =~ ^[Nn]$ ]]; then
                            ...
                            read -r
                            enhanced_session_end "$post_file" "$current_site"
                        fi
                    else
                ...
                p|publish)
                    ...
                    if post_file=$(select_completed "$current_site"); then
                        ...
                        publish_post "$title" "$current_site"
                        echo ""
                        echo -ne "${BLUE}Press any key to continue...${NC}"
                        read -r
                    fi
                ...
                m|promote)
                    if [[ "$current_site" == "dsc" ]]; then
                        echo -e "${CYAN}📤 Publishing draft to dated folder...${NC}"
                        echo ""
                        ...
                        echo -e "${RED}❌ Draft publishing is only available for Digital Sovereignty Chronicle${NC}"
                        echo -ne "${BLUE}Press any key to continue...${NC}"
                        read -r
                    fi
                    ;;
...
[a-z])
                    ...
                    # Get the list of completed posts and select by letter (site-specific)
                    local completed=()
                    if [[ "$current_site" == "dsc" ]]; then
                        while IFS= read -r line; do
                            if [ -n "$line" ]; then
                    ...
                    if [ "$letter_idx" -ge 0 ] && [ "$letter_idx" -lt ${#completed[@]} ]; then
                        ...
                        if [ -f "$post_file" ]; then
                            ...
                            publish_post "$title_for_publish" "$current_site"
                            echo ""
                            echo -ne "${BLUE}Press any key to continue...${NC}"
                            read -r
                        else
